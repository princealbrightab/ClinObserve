...
<div class="content-card"><form class="row g-3 mb-4" method="get"><div class="col-md-4"><x-field name="month" type="month" :value="$month->format('Y-m')" label="Month"/></div>@can('faculty')<div class="col-md-5"><label for="student_id" class="form-label">Student</label><select class="form-select" name="student_id" id="student_id"><option value="">All students</option>@foreach($students as $student)<option value="{{ $student->id }}" @selected((string)$studentId===(string)$student->id)>{{ $student->name }}</option>@endforeach</select></div>@endcan<div class="col-md-3 d-flex align-items-center"><button class="btn btn-primary">Show calendar</button></div></form>
<div class="section-heading"><a href="{{ route(request()->route()->getName(),['month'=>$month->subMonth()->format('Y-m'),'student_id'=>auth()->user()->isFaculty()?$studentId:null]) }}">← Previous</a><h2>{{ $month->format('F Y') }}</h2><a href="{{ route(request()->route()->getName(),['month'=>$month->addMonth()->format('Y-m'),'student_id'=>auth()->user()->isFaculty()?$studentId:null]) }}">Next →</a></div>
<div class="calendar-grid">@foreach(['Mon','Tue','Wed','Thu','Fri','Sat','Sun'] as $day)<div class="calendar-label">{{ $day }}</div>@endforeach
@foreach($days as $day)<a class="calendar-day {{ $day['date']->month!==$month->month?'outside':'' }} {{ $selected===$day['date']->toDateString()?'selected':'' }}" href="{{ route(request()->route()->getName(),['month'=>$month->format('Y-m'),'date'=>$day['date']->toDateString(),'student_id'=>auth()->user()->isFaculty()?$studentId:null]) }}"><strong>{{ $day['date']->day }}</strong>@if($day['count'])<span>{{ $day['count'] }} <span class="d-none d-sm-inline">encounters</span></span>@endif</a>@endforeach</div></div>
