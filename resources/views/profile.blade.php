...
<label for="avatar" class="form-label">Profile photo (JPEG, PNG or WebP; max 2 MiB)</label><input class="form-control mb-4" name="avatar" id="avatar" type="file" accept="image/jpeg,image/png,image/webp"><button class="btn btn-primary">Save profile</button></form>@else<div class="content-card"><h2>{{ $user->isHod()?'Head of department':($user->isProfessor()?'Professor':'Academic profile') }}</h2><p class="text-muted">{{ $user->isHod()?'Your faculty account can manage students and review academic observations.':($user->isProfessor()?'Faculty account':'Ask your HOD to complete your academic profile.') }}</p></div>@endif
@endsection
