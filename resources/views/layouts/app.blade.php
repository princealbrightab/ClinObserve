...
@can('hod')<a class="{{ request()->routeIs('hod.students.*')?'active':'' }}" href="{{ route('hod.students.index') }}">♙ <span>Students</span></a>@endcan
<a class="{{ request()->routeIs('patients.*')?'active':'' }}" href="{{ route('patients.index') }}">▤ <span>Patient cases</span></a>
<a class="{{ request()->routeIs('encounters.*')?'active':'' }}" href="{{ route('encounters.index') }}">▧ <span>{{ auth()->user()->isHod()?'Observations':'My observations' }}</span></a>
