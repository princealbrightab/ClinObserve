...
<div class="mb-3"><label class="form-label" for="professor_id">Assigned professor</label><select class="form-select" name="professor_id" id="professor_id"><option value="">Unassigned</option>@foreach($professors as $professor)<option value="{{ $professor->id }}" @selected((string)old('professor_id', $student->professor_id) === (string)$professor->id)>{{ $professor->name }}{{ $professor->is_active ? '' : ' (Inactive)' }}</option>@endforeach</select></div>
