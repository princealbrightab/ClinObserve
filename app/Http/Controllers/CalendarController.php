...
class CalendarController extends Controller
{
    public function __invoke(CalendarRequest $request): View
    {
        ...
        $studentId = $request->user()->isFaculty() ? $request->validated('student_id') : $request->user()->id;
        $query = PatientEncounter::with(['patient', 'student'])->forWorkspace($request->user())->when($studentId, fn ($q) => $q->where('student_id', $studentId));
        ...
        $students = $request->user()->isFaculty() ? User::visibleStudents($request->user())->orderBy('name')->get(['id', 'name']) : collect();
